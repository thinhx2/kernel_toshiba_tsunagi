...
static int debug_set_stat(void *data, u64 val)
{
	printk("%s\n", debug_set_stat);
	smsm_change_state(SMSM_STATE_MODEM,
			  0, SMSM_RPCINIT);
	printk("SMSM_STATE_MODEM 0x%08x\n", raw_smsm_get_state(SMSM_STATE_MODEM));
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(smd_dbg_set_fops,
			NULL,
			debug_set_stat, "%llu\n");
...
static int __init smd_debugfs_init(void)
{
	...
	if (IS_ERR(dent))
		return -1;
	debugfs_create_file("set", 0666, dent, NULL,
			    &smd_dbg_set_fops);
